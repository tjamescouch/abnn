...
public:
    EmbeddingLayer(int vocabSize, int embeddingDim, int sequenceLength, int outputDim, int batchSize);
    ~EmbeddingLayer();
    ...
    EmbeddingLayer* setLearningRate(float learningRate) { learningRate_ = learningRate; return this; }
    EmbeddingLayer* setInitializer(std::string initializer) { initializer_ = initializer; return this; }
...
private:
    ...
    std::unique_ptr<Optimizer> optimizerEmbeddings_;
